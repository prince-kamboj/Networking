...
Subnetting is an important security practice because it allows organizations to isolate systems and limit access.

Examples:

- Separate employee and guest Wi-Fi networks.
- Place public web servers in a DMZ subnet.
- Isolate sensitive databases from user devices.
- Restrict communication between departments using firewalls or ACLs.
- Reduce the impact of malware by limiting its ability to spread across the entire network.

Network segmentation through subnetting is a key component of a defense-in-depth strategy.
...
To improve network performance, security, organization, and efficient IP address usage.
...
Classless Inter-Domain Routing, a notation that uses a prefix length (such as `/24`) instead of traditional address classes.
...
Because one address is reserved as the **network address** and one as the **broadcast address**.

---

### Which subnet mask corresponds to `/24`?
...
- It reduces broadcast traffic and improves performance.
- CIDR notation represents subnet masks using prefix lengths.
- The first IP in a subnet is the network address.
- The last IP in a subnet is the broadcast address.
- Usable host addresses lie between the network and broadcast addresses.
- Borrowing host bits creates more subnets but fewer hosts per subnet.
- Network segmentation using subnetting improves both management and security.